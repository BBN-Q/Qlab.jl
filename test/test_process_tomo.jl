...
"""
Create the quantum process tomography problem for a given number of qubits

This setup defaults to the obervables expected in the python QGL package and
assumed in tht rest of the code base.

Returns a process matrix, the set of initial states, the set of observables,
the mean values of those observables and the variance in the case of least
squares mean values.

If asymptotic is true, return the true predicted means.  If false, add noise.
"""
function qlab_qpt_setup(num_Qubits;num_obs = 6,
                                   asymptotic=false,
                                   n = 10_000)
    ...
    # create a random num_Qubit process
    u = rand(ClosedHaarEnsemble(num_Qubits^2))
    E = liou(rand(RandomQuantum.ClosedHaarEnsemble(num_Qubits^2)))

    # only handle one and two qubit processes
    prep = map(projector, _obs )
    if num_Qubits == 1
        preps = prep
    elseif num_Qubits == 2
        # in the two-qubit case, we need the outer product of the prep unitaries
        preps = Matrix{ComplexF64}[]
        for p in prep
            append!(preps, map(x -> kron(p,x), prep))
        end
    else
        @warn("Code only handles one and two-qubit processes!")
    end
    obs = map(ψ->projector(u*ψ), obs_ )
