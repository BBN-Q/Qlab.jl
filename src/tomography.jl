...
# Arguments
- `data`: data array structured as a set of data with a string name with a 'data'
...
"""
    ProcessTomoBootstrap((data::Dict{String,Dict{String,Array{Any,N} where N}},
                            nbrQubits,
                            nbrPrepPulses,
                            nbrReadoutPulses,
                            nbrCalRepeats=2,
                            nbrSamples=1000,
                            nbrAverages = 10000)
Function to estimate confidence intervals on process tomography via bootstrapping.
...
- `data`: data array structured as a set of data with a string name with a 'data'
          ...
          required for reconstruction.
- `nbrQubits`: number of qubits.
- `nbrPrepPulses`: number of preparation pulses.  Either 4 or 6.  12 is possible but left
             for the user to do manually.
- `nbrReadoutPulses`: number of readout pulses.  Either 4 or 6.  12 is possible but left
             for the user to do manually.
- `nbrCalRepeats`: number of repeated calibration points per calibration state.
- `nbrSamples`: Number of resamplings to run for estimation.
- `nbrAverages`: Number of averages taken in the tomography experiment.
# Returns
- `choi_rand` : an array of nbrSamples process matrices.
...
function ProcessTomoBootstrap(data::Dict{String,Dict{String,Array{Any,N} where N}},
                            nbrQubits,
                            nbrPrepPulses,
                            nbrReadoutPulses,
                            nbrCalRepeats=2,
    ...
    for k = 1:nbrSamples
        for h = 1:length(tomoData)
            ...
            TomoSample[h,k] = tomoData[h] + sqrt(varData[h]./nbrAverages).*randn()  #Generate random samples and average. 
    ...
    choi_rand = Matrix{Complex}[]

    for k = 1:nbrSamples

        choiLSQ = QPT_LSQ(TomoSample[:,k], varData, measPulseMap, measOpMap, prepPulseUs, measPulseUs, measOps, nbrQubits,nbrPulses)

        push!(choi_rand,choiLSQ)
    end
